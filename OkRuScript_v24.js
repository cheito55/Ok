...
function extractSearchResults(html) {
     ...
     */
    var anchorRe = /<a\b([^>]*?href\s*=\s*["'](?:https?:\/\/[^"']+)?(?:\/)?video\/(\d+)(?:[?#][^"']*)?["'][^>]*)>([\s\S]*?)<\/a>/gi;
    var m;
    ...
    while ((m = anchorRe.exec(html)) !== null && results.length < MAX_SEARCH) {
        ...
        var blockStart = Math.max(0, m.index - 1200);
        var blockEnd = Math.min(html.length, anchorRe.lastIndex + 1800);
        var block = html.substring(blockStart, blockEnd);

        var poster = "";
        var pm = block.match(/(?:poster|thumbnail|data-poster|og:image)[^>:=]{0,80}[=:]\s*["']([^"']+)["']/i);
        if (pm) poster = normalizeUrl(pm[1]);

        /* Look for an image URL in the same result block. */
        if (!poster) {
            var im = block.match(/<img[^>]+(?:src|data-src|data-lazy-src)\s*=\s*["']([^"']+)["']/i);
            if (im) poster = normalizeUrl(im[1]);
        }

        var duration = 0;
        var dm = block.match(/(?:duration|movie-duration|video-duration)[^>]*>[\s\S]{0,80}?([0-9]{1,2}:[0-9]{2}(?::[0-9]{2})?)/i);
     ...
     */
    if (results.length === 0) {
        var re = /data-movie-id\s*=\s*["']?(\d+)["']?([\s\S]{0,5000}?)(?=data-movie-id|$)/gi;
        while ((m = re.exec(html)) !== null && results.length < MAX_SEARCH) {
            ...
            var tm = block.match(/(?:data-title|title)\s*=\s*["']([^"']+)["']/i);
            if (tm) title = cleanText(tm[1]);
            if (!title) {
                tm = block.match(/<(?:span|div|a)[^>]*class=["'][^"']*(?:title|name)[^"']*["'][^>]*>([\s\S]{1,500}?)<\/(?:span|div|a)>/i);
                if (tm) title = cleanText(tm[1]);
            }
            var pm = block.match(/(?:poster|thumbnail|data-poster|data-options)\s*=\s*["']([^"']+)["']/i);
            if (pm) poster = normalizeUrl(pm[1]);
            var dm = block.match(/(?:duration|movie-duration)[^>]*>([^<]{1,30})</i);
            if (dm) duration = parseDurationText(dm[1]);
...
function extractPageTitlePoster(html, pageUrl) {
    ...
    /* Prefer the same metadata users see on the OK.ru video page. */
    ...
    if (!out.title) {
        m = html.match(/<meta[^>]+name=["']title["'][^>]+content=["']([^"']+)["'][^>]*>/i);
    ...
    if (!out.poster) {
        m = html.match(/(?:poster|thumbnailUrl|thumbnail|previewImage|coverUrl)\s*[:=]\s*["']([^"']+)["']/i);
        if (m) out.poster = normalizeUrl(m[1], pageUrl);
    }

    var dm = html.match(/(?:duration|movie-duration|video-duration)[^>:=]{0,100}[=:>\s]+["']?([0-9]{1,2}:[0-9]{2}(?::[0-9]{2})?)/i);
    if (dm) out.duration = parseDurationText(dm[1]);
...
function enrichSearchResult(r) {
    if (!r || !r.url) return r;
    try {
        var html = loadOkPage(r.url);
        if (!html) return r;

        var page = extractPageTitlePoster(html, r.url);
        var meta = parseMetadata(html, r.url) || {};

        var title = page.title || getTitle(meta, "");
        var poster = page.poster || getPoster(meta, r.url);
        var duration = r.duration || page.duration || getDuration(meta);

        /* Never show the internal OK.ru ID as the user-facing name when the
           public page exposes a real title. */
        if (title && !/^OK\\.ru video \\d+$/i.test(title)) r.title = title;
        if (poster) r.thumbnail = poster;
        if (duration) r.duration = duration;
    } catch (e) {
        addDebug("search enrich " + r.id + "=" + e);
    }
    return r;
}
...
function searchOk(query) {
    ...
    var url = SEARCH_URL_BASE + encodeURIComponent(q);
    ...
    /* The anonymous search page often exposes only the numeric ID. Resolve
       each result's public page once so GrayJay receives the real title and
       poster instead of "OK.ru video 123...". */
    for (var i = 0; i < raw.length && i < MAX_SEARCH; i++) {
        var r = raw[i];
        if (!r.title || /^OK\.ru video \d+$/i.test(r.title) || !r.thumbnail) {
            enrichSearchResult(r);
        }
...
function searchSuggestions(query) {
    var out = [];
    try {
        var url = SEARCH_URL_BASE + encodeURIComponent(safeStr(query));
        var html = httpGetAuth(url);
        if (!html) html = httpGet(url);
        if (!html) return out;
        var raw = extractSearchResults(html);
