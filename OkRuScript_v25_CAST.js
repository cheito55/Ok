...
function fetchMetadataUrl(meta, pageUrl) {
    ...
    for (var i = 0; i < candidates.length; i++) {
        ...
        /* Algunas variantes de OK.ru requieren POST al metadataUrl. */
        try {
            var headers = {
                "User-Agent": UA_DESKTOP,
                "Accept": "application/json,text/plain,*/*",
                "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
                "Referer": pageUrl || "https://ok.ru/"
            };
            var pr = http.POST(url, "", headers);
            if (pr) {
                var pb = "";
                try { pb = pr.body; } catch (_) {}
                if (!pb) {
                    try { pb = pr.getBody(); } catch (_) {}
                }
                var pobj = tryParseJson(pb);
                if (pobj) return findMetadata(pobj, 0) || pobj;
            }
        } catch (_) {}
    }
...
function searchOk(
    ...
    try {
        if (continuationToken && typeof continuationToken === "object") {
            page = Number(continuationToken.page) || 1;
        } else if (continuationToken) {
            page = Number(continuationToken) || 1;
        }
    } catch (_) {}

    var q = safeStr(query);
    var urls = [
        "https://ok.ru/search/content?q=" + encodeURIComponent(q),
        "https://ok.ru/video/search?st.cmd=anonymVideo&st.ft=search&st.gsq=" + encodeURIComponent(q),
        "https://ok.ru/dk?st.cmd=searchResult&st.mode=Movie&st.grmode=Groups&st.query=" + encodeURIComponent(q)
    ];

    var raw = [];
    var seen = {};

    for (var u = 0; u < urls.length && raw.length < MAX_RESULTS; u++) {
        var html = loadPage(urls[u]);
        if (!html || html.length < 200) continue;

        var part = extractSearchResults(html);
        for (var i = 0; i < part.length && raw.length < MAX_RESULTS; i++) {
            if (!seen[part[i].id]) {
                seen[part[i].id] = true;
                raw.push(part[i]);
            }
    ...
    if (!raw.length) {
        throw new Error("OK.ru no devolvió resultados de búsqueda");
    }

    var out = [];
    for (var j = 0; j < raw.length && out.length < MAX_RESULTS; j++) {
        var item = enrichSearchResult(raw[j]);
        var v = makeSearchVideo(item);
        if (v) out.push(v);
    }
